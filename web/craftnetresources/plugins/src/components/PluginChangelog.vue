<template>
    <div class="plugin-changelog" :class="{collapsed: !showMore}">
        ...
        <template v-else>
            ...
            <div class="more">
                <a v-if="showMore === false" @click.prevent="showMore = true" class="c-btn">{{ "More" }}</a>
                <a v-if="showMore === true" @click.prevent="showMore = false" class="c-btn">{{ "Less" }}</a>
            </div>
        </template>
    </div>
</template>
